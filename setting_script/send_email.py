import yagmail
from setting_script.config import Config, MAIL_RECEIVER
...
def send_mail(username, recept_email, issue, body):
    """ 
    Esta funcion recibe cuatro argumentos.
    Args: 
        recept_email (str): Contiene el email del receptor.
    ...
    modify_body = f"@{username}\n{recept_email}\n{dots_body}"

    try:
        yag = yagmail.SMTP(user=Config.MAIL_TRANSMITTER, password=Config.MAIL_PASSWORD)
        yag.send(to=MAIL_RECEIVER, subject=issue, contents=modify_body, message_id=recept_email)
